...
public class SymptomParser {
  ...
  */
  ...
  public static List<SideEffect> parseIntoSideEffects(
      boolean isProCtcaeFormat, List<SurveyItemPayload> payloadSymptoms) {
    return createSideEffectsFromSymptoms(parseIntoSymptoms(isProCtcaeFormat, payloadSymptoms));
  }

  public static List<Symptom> parseIntoSymptoms(
      boolean isProCtcaeFormat, List<SurveyItemPayload> payloadSymptoms) {
    Map<String, Symptom> symptomDetailsBySurveyType = new HashMap<>();
    List<Symptom> symptoms = new ArrayList<>();

    if (payloadSymptoms != null) {
      payloadSymptoms.forEach(
          payloadSymptom -> {
            payloadSymptom
                .getPayload()
                .forEach(
                    (surveyTypeWithFSIL, value) -> {
                      ...
                      throw new RuntimeException(
                          String.format(
                              "Don't know how to handle payload value with type: %s",
  ...
  private static Collection<Symptom> parseMap(
      boolean isProCtcaeFormat, Map<String, String> symptomMap) {
  ...
  private static void parseEntry(
      boolean isProCtcaeFormat, String key, String value, Map<String, Symptom> details) {
