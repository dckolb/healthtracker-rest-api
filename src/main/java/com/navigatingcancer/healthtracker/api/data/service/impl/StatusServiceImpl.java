...
public class StatusServiceImpl implements StatusService {
    ...
    @Autowired
    private HealthTrackerStatusRepository statusRepository;
    ...
    @Override
    public List<HealthTrackerStatusResponse> getManualCollectDueByIds(Long clinicId, List<String> ids) {
        ...
        statusRepository.findByIds(clinicId, dueIds).forEach(status -> {
            ...
            if (checkInData != null) {
                ...
                log.debug("enrollment: {}", enrollment);
                results.add(new HealthTrackerStatusResponse(status, checkInData.getEnrollment(), checkInData));
            }
        });
        return results;
    }
    ...
    public List<String> findCheckInsDue(List<String> enrollmentIds) {
        ...
        resultIds.addAll(customCheckInRepository.findCheckIns(enrollmentIds, symptomStatuses, CheckInType.SYMPTOM));
        resultIds.addAll(customCheckInRepository.findCheckIns(enrollmentIds, oralStatuses, CheckInType.ORAL));
