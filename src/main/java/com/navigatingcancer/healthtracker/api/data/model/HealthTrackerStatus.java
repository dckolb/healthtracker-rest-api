...
public class HealthTrackerStatus extends AbstractDocument {
  ...
  // NOTE : There are strict rules for adding RuleResults so all addition should be done via the
  // addResult method.
    ...
    switch (result.getType()) {
      ...
      case noAction:
        ...
        if (ruleResults.isEmpty()
            || (ruleResults.size() == 1
                && ruleResults.stream().anyMatch(c -> c.isParticipation() || c.isNoAction()))) {
  ...
  public void addResultAndSetTriageReason(
      CheckInResult.ResultType type,
      boolean passed,
      String desc,
      String note,
