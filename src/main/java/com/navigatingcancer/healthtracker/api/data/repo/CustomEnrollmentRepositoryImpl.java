 ...
 *
 * <p>This class is instantiated by Spring Data autoconfiguration since the EnrollmentRepository
 * extends the CustomEnrollmentRepository and this implementation of that interface is suffixed with
 * 'Impl'
 */
@Slf4j
@Repository
public class CustomEnrollmentRepositoryImpl implements CustomEnrollmentRepository {
  ...
  public Boolean activeEnrollmentExists(long patientId, long clinicId) {
    ...
    Criteria criteria =
        Criteria.where("clinicId")
            .is(clinicId)
            .and("patientId")
            .is(patientId)
            .and("status")
            .in(Arrays.asList(EnrollmentStatus.ACTIVE, EnrollmentStatus.PAUSED));
  ...
  public List<Enrollment> findEnrollments(EnrollmentQuery params) {
    ...
    if (params.getClinicId() != null && params.getClinicId().size() > 0) {
      criteria = Criteria.where("clinicId").in(params.getClinicId());
    }
    if (params.getId() != null && params.getId().size() > 0) {
      if (criteria == null) criteria = Criteria.where("id").in(params.getId());
      else criteria.and("id").in(params.getId());
    }
    if (params.getLocationId() != null && params.getLocationId().size() > 0) {
      if (criteria == null) criteria = Criteria.where("locationId").in(params.getLocationId());
      else criteria.and("locationId").in(params.getLocationId());
    }
    if (params.getPatientId() != null && params.getPatientId().size() > 0) {
      if (criteria == null) criteria = Criteria.where("patientId").in(params.getPatientId());
      else criteria.and("patientId").in(params.getPatientId());
    }
    if (params.getStatus() != null && params.getStatus().size() > 0) {
      if (criteria == null) criteria = Criteria.where("status").in(params.getStatus());
      else criteria.and("status").in(params.getStatus());
    }
    if (params.getEndDate() != null && params.getStartDate() != null) {
      if (criteria == null)
        criteria =
            Criteria.where("txStartDate").gte(params.getStartDate()).lte(params.getEndDate());
    }

    if (criteria == null) {
      log.error("no known parameters sent in findEnrollments query");
  ...
  public List<Enrollment> getCurrentEnrollments(
    ...
    List<Criteria> commonAndCriterion =
        getAndCriteriaForCurrentEnrollments(locationIds, providerIds, isManualCollect);
    ...
    log.info("Query used for filtering is {}", query);
  ...
  private void addCriterionForManualCollect(Criteria criteria, List<Criteria> commonAndCriterion) {
    ...
    Criteria c1And =
        c1.andOperator(
            Criteria.where("reminderStartDate")
                .exists(false), // if reminderStartDate exists then use that one
    ...
    Criteria c3Or = c3.orOperator(c2And, c1And);
  ...
  private Criteria[] getOrCriteriaForCurrentEnrollments() {
    ...
    orCriterion.add(
        Criteria.where("status").nin(EnrollmentStatus.COMPLETED, EnrollmentStatus.STOPPED));
  ...
  private List<Criteria> getAndCriteriaForCurrentEnrollments(
      List<Long> locationIds, List<Long> providerIds, Boolean isManualCollect) {
    ...
    if (locationIds != null && !locationIds.isEmpty()) {
      ...
      if (locationIds.contains(0L)) {
        ...
        c.orOperator(
            Criteria.where("locationId").in(locationIds),
    ...
    if (providerIds != null && !providerIds.isEmpty()) {
      ...
      if (providerIds.contains(0L)) {
        Criteria c = new Criteria();
        c.orOperator(
            Criteria.where("providerId").in(providerIds),
  ...
  public Enrollment setStatus(String id, EnrollmentStatus status) {
    ...
    return template.findAndModify(
        query, update, new FindAndModifyOptions().returnNew(true), Enrollment.class);
  }

  @Override
  public Enrollment updateConsentStatus(
      String consentRequestId, String consentStatus, Date updatedDate) {
    Query query = new Query(Criteria.where("consentRequestId").is(consentRequestId));
    Update update =
        new Update().set("consentStatus", consentStatus).set("consentUpdatedDate", updatedDate);
    return template.findAndModify(
        query, update, new FindAndModifyOptions().returnNew(true), Enrollment.class);
  }
  ...
  @Override
  public boolean updateCheckInScheduleByCheckInType(
      Enrollment enrollment, CheckInSchedule schedule) {
    Preconditions.checkNotNull(
        schedule.getCheckInType(),
        "cannot update schedule by check in type if it does not have one");
    ...
    var update =
        template.updateFirst(
            new Query(
                Criteria.where("_id")
                    .is(enrollment.getId())
                    .and("schedules.checkInType")
                    .is(schedule.getCheckInType())),
            new Update().set("schedules.$", schedule),
            Enrollment.class);
  ...
  @Override
  public CheckInSchedule getCheckInScheduleById(String checkInScheduleId) {
    var query = new Query(Criteria.where("schedules._id").is(checkInScheduleId));
    query.fields().include("schedules.$");
    var enrollment = template.findOne(query, Enrollment.class);
    var schedule = enrollment.getSchedules().get(0);
