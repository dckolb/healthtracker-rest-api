...
public class ConsentServiceImpl implements ConsentService {
    ...
    @Autowired
    public ConsentServiceImpl(DocusignService docusignService,
    ...
    @Override
    public String processSurveyConsentForEnrollment(ProgramConfig.ProgramConsent consent, Enrollment enrollment) {
        if (consent == null ) return null;

        if (ConsentType.DOCUSIGN.name().equalsIgnoreCase(consent.getType())) {
            List<PatientInfo> patients = this.patientInfoClient.getApi().getPatients(enrollment.getClinicId(), enrollment.getPatientId());
            ...
            ProgramConfig.ConsentContent consentContent = consent.getConsentContent().get(enrollment.getDefaultLanguage().toString());
            ...
            for (String role : consent.getRoles()){
                ...
                signingRequestRole.setName(String.format("%s %s", patientInfo.getFirstName(), patientInfo.getLastName()));
                signingRequestRole.setEmail(enrollment.getEmailAddress() != null ?
