...
public class EnrollmentStatusTest {
  ...
  private Enrollment createEnrollment(
      Long clinicId, Long locationId, Long patientId, LocalDate dt, SurveyItemPayload... items) {
    return HealthTrackerStatusServiceTest.createEnrollment(
        enrollmentRepository, checkinRepository, clinicId, locationId, patientId, dt, items);
  }
  ...
  public void testContinuousEnrollmentIsActive() {
    ...
    TriggerPayload triggerPayload =
        new TriggerPayload(e.getId(), CheckInType.ORAL, LocalTime.of(9, 0), TriggerType.STATUS);
  ...
  public void testOldEnrollmentGetsDeactivated() {
    ...
    LocalDate pastDate = LocalDate.of(2019, 01, 01);
    LocalDate enrollmentStartDate = pastDate.minusDays(10);
    LocalDate enrollmentCheckDate =
        pastDate.plusDays(daysInEnrollment + 1); // Some day past the end date
  ...
  public void testEnrollmentCheckOnLastDate() {
    ...
    TriggerPayload triggerPayload =
        new TriggerPayload(e.getId(), CheckInType.ORAL, LocalTime.of(9, 0), TriggerType.STATUS);
    Date enrollmentPenultimateDateTs =
        Timestamp.valueOf(LocalDateTime.of(enrollmentLastDate.minusDays(1), fireTime));
    TriggerEvent triggerEvent =
        new TriggerEvent(JsonUtils.toJson(triggerPayload), enrollmentPenultimateDateTs, null);
    schedulingService.accept(triggerEvent);
