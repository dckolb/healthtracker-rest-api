...
public class CheckInAggregatorTest {
    ...
    private CheckInAggregator createCheckInAggregator(boolean isHighRisk, String... itemPayloads) {
        ...
        List<CheckIn> checkIns = DefaultDroolsServiceTest.createCheckInsFromSurveyPayload(
    ...
    public void countAnswerAtLeastModerate_withInvalidInputs() {
        ...
        Assert.assertEquals("0 for invalid input", 0, cia.countAnswerForMandatoryProCtcaeSymptoms(-1, "1,2,3"));
        Assert.assertEquals("0 for invalid input", 0, cia.countAnswerForMandatoryProCtcaeSymptoms(1, ""));
    }
    ...
    public void countAnswerAtLeastModerate_withLessThanModerateResponse() {
        ...
        Assert.assertEquals("0 for response < 2", 0, cia.countAnswerForMandatoryProCtcaeSymptoms(0, "2,3,4"));
    }
    ...
    public void countAnswerAtLeastModerate_withEqualToModerateResponse() {
        ...
        Assert.assertEquals("1 count for response == 2", 1, cia.countAnswerForMandatoryProCtcaeSymptoms(0, "2,3,4"));
    }
    ...
    public void addProCtcaeSymptom_withModerateResponse() {
        ...
        for(CheckInResult rule : rules) {
            ...
            Assert.assertEquals("Rule triage reason type is 'symptom'", "symptom", rule.getTriageReasonType());
            Assert.assertEquals("Rule triage severity is 'Moderate'", "moderate", rule.getTriageSeverity());
            Assert.assertEquals("Rule triage symptom type is 'nausea'", "nausea", rule.getTriageSymptomType());
        }
    }
    ...
    public void addProCtcaeSymptom_withVerySevereResponse() {
        ...
        for(CheckInResult rule : rules) {
            ...
            Assert.assertTrue("Note is empty", rule.getNote().isEmpty());
            Assert.assertEquals("Rule triage reason type is 'symptom'", "symptom", rule.getTriageReasonType());
            Assert.assertEquals("Rule triage severity is 'Very Severe'", "severe", rule.getTriageSeverity());
            Assert.assertEquals("Rule triage symptom type is 'nausea'", "nausea", rule.getTriageSymptomType());
        }
    }
    ...
    public void addProCtcaeSymptom_withAlmostConstantFrequency() {
        ...
        for(CheckInResult rule : rules) {
            ...
            Assert.assertEquals("Rule title is valid", "Constipation occurs almost constantly", rule.getTitle());
            Assert.assertTrue("Note is empty", rule.getNote().isEmpty());
            Assert.assertEquals("Rule triage reason type is valid", "symptom", rule.getTriageReasonType());
            Assert.assertEquals("Rule triage severity is valid", "severe", rule.getTriageSeverity());
            Assert.assertEquals("Rule triage symptom type is valid", "constipation", rule.getTriageSymptomType());
        }
    }

    @Test
    public void addProCtcaeSymptom_withMultipleTriggeringAnswers() {
        CheckInAggregator cia = createCheckInAggregator(false,
                "constipationFrequency", "4", "nauseaSeverity", "2",
                "painSeverity", "2", "painFrequency", "3", "painInterference", "4");
        cia.addProCtcaeSymptom(0, "2,3,4");
        Set<CheckInResult> rules = cia.getHtStatus().getRuleResults();
    ...
    public void givenMissedCheckins_shouldReturnConsecutiveCheckinsMissed() {
        List<CheckIn> checkIns = new ArrayList<>();
        checkIns.add(DefaultDroolsServiceTest.createCheckInsFromSurveyPayload(CheckInType.ORAL, LocalDate.now().minusDays(1L), null));
        checkIns.add(DefaultDroolsServiceTest.createCheckInsFromSurveyPayload(CheckInType.SYMPTOM, LocalDate.now().minusDays(1L), null));
        checkIns.add(DefaultDroolsServiceTest.createCheckInsFromSurveyPayload(CheckInType.ORAL, LocalDate.now().minusDays(3L), null));
        checkIns.add(DefaultDroolsServiceTest.createCheckInsFromSurveyPayload(CheckInType.SYMPTOM, LocalDate.now().minusDays(3L), null));
        checkIns.add(DefaultDroolsServiceTest.createCheckInsFromSurveyPayload(CheckInType.ORAL, LocalDate.now().minusDays(5L),
                DefaultDroolsServiceTest.createSurvey("medicationTaken", "true")));
        checkIns.add(DefaultDroolsServiceTest.createCheckInsFromSurveyPayload(CheckInType.ORAL, LocalDate.now().minusDays(8L), null));
